...
export class ScheduleComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    ...
    const message = 'Este perfil no tiene un horario disponible.';
    this.errorMessage = message;
    await this.feedback.info(message);
  }
  ...
  getSubtitle(): string {
    if (this.isTeacher) return `${this.getTodayLabel()} • Horario docente`;
    const group = this.user?.studentProfile?.group?.name;
    return group ? `${this.getSelectedDay()} • ${group}` : this.getSelectedDay();
  }
  ...
  private loadStudentSchedule(): void {
    ...
    this.scheduleService.getStudentSchedule(studentId).subscribe({
      ...
      error: async (error: any) => {
        this.loading = false;
        const message = error?.error?.message ?? 'No se pudo cargar el horario.';
        this.errorMessage = message;
        await this.feedback.error(message);
      },
    });
  ...
  private loadTeacherSchedule(): void {
    const teacherId = this.user?.teacherProfile?.id ?? this.user?.teacherProfileId ?? this.user?.teacherId;
    if (!teacherId) {
    ...
    this.scheduleService.getTeacherSchedule(teacherId).subscribe({
      next: async (response: any[]) => {
        ...
        if (this.teacherClasses.length === 0) {
          await this.feedback.info('No tienes clases programadas para hoy.');
        }
      },
      error: async (error: any) => {
        this.loading = false;
        this.teacherClasses = [];
        const message = error?.error?.message ?? 'No se pudo cargar el horario del docente.';
        this.errorMessage = message;
        await this.feedback.error(message);
      },
    });
  ...
  private mapSchedule(schedule: any): ClassItem {
